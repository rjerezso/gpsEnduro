...
// Crear icono de vehículo con flecha direccional para navegación (tipo moto/GPS)
...
// Variables para la navegación avanzada
let currentHeading = 0;
let lastHeading = 0;
let mapRotation = 0;
let cumulativeRotation = 0; // Variable para rotación suave continuada
let vehicleMarker = null;
let billboardTimeout = null;
let navigationStartTime = 0; // Para el grace period

// Funciones para rotación del mapa tipo GPS
function setMapRotation(degrees) {
    ...
    // Normalizar delta al camino más corto (-180 a 180)
    ...
    // APLICAR TRANSFORMACIÓN AL CONTENEDOR DEL MAPA (NO AL PANE)
    // Usamos cumulativeRotation en lugar de degrees absoluto para evitar saltos
    // Añadido soporte explícito para WebKit (iOS)
    const transformString = `perspective(1000px) rotateX(60deg) rotateZ(${-cumulativeRotation}deg) scale(1.5)`;
    mapDiv.style.webkitTransform = transformString;
    mapDiv.style.transform = transformString;
}
...
function enableGPSView() {
    ...
    // CAMBIAR A MODO SATÉLITE AUTOMÁTICAMENTE PARA REALISMO 3D
...
function disableGPSView() {
    // Desactivar rotación y volver a norte arriba
    const mapDiv = document.getElementById('map');
...
function saveRoute() {
    if (!markerA || !markerB || routeCoordinates.length === 0) {
        showNotification('⚠️ Primero calcula una ruta');
        return;
    }
        ...
        list.innerHTML = savedRoutes.map(route => `
                    <div class="flex items-center gap-2 bg-gray-700 rounded-lg p-2">
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-gray-400 text-xs truncate">${route.startName} → ${route.endName}</p>
...
function loadRoute(routeId) {
    ...
    if (!route) {
        showNotification('⚠️ Ruta no encontrada');
        return;
    }
    ...
    showNotification(`✅ Ruta "${route.name}" cargada`);
}
...
function deleteRoute(routeId) {
    ...
    loadSavedRoutes();
    showNotification('🗑️ Ruta eliminada');
}
...
`;
    ...
    // Añadir waypoints de inicio y fin
    if (markerA && markerB) {
...
`;
    ...
    showNotification('📥 Archivo GPX descargado');
}
...
async function reverseGeocode(latlng, point) {
    ...
    input.value = '📍 Cargando dirección...';
...
async function calculateRoute() {
    if (!markerA || !markerB) {
        showNotification('⚠️ Marca ambos puntos (A y B) en el mapa');
        return;
    }
    ...
    try {
        ...
        showNotification('⚠️ Error al calcular ruta. Inténtalo de nuevo.');
    }
...
async function calculateOffroadRoute(startCoords, endCoords) {
    ...
    // También intentar rutas alternativas con waypoints
...
// Generar steps de navegación desde coordenadas
function generateStepsFromCoords(coords, messages) {
    const steps = [];

    // Calcular giros basados en cambios de dirección en las coordenadas
    for (let i = 0; i < coords.length; i++) {
        const coord = coords[i];

        // Calcular dirección si hay punto anterior y siguiente
        ...
        // Solo añadir steps cada ciertos puntos o si hay giro
    ...
    for (const off of offsets) {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                if (data.routes && data.routes.length > 0) {
                    // Solo añadir si es significativamente diferente
...
function displayRoute(route, index, total) {
    ...
    // Calcular duración - siempre estimar basado en distancia y velocidad
    ...
    // Ajustar para off-road (terreno más difícil)
    if (routeType === 'offroad') {
        duration = Math.round(duration * 1.2);
    }

    // Mostrar desnivel real si está disponible (BRouter)
    let elevation;
    if (route.ascend) {
    ...
    if (total > 1) {
        showNotification(`🛤️ Ruta ${index}/${total}${profileText} - ¡Pulsa para ver otra!`);
    } else {
        showNotification(`✅ Ruta calculada${profileText} - ¡Pulsa INICIAR!`);
    }
}
...
function startNavigation() {
    ...
    setTimeout(() => {
        map.invalidateSize();
        map.setView(startPoint, 18); // Zoom más cercano para navegación

        // Crear marcador de vehículo en el punto inicial
...
// ... (handleLocationError is here in between) ...
...
function stopNavigation() {
    ...
    showNotification('🛑 Navegación finalizada');
}
function updateNavigation(position) {
    ...
    // El icono del vehículo rota según heading
    // Rotar mapa si la velocidad es > 5 km/h para efecto Waze
    if (currentSpeed > 5) {
...
function showTurnBillboard(turnType, modifier, distance, streetName) {
    ...
    // Detectar giro (múltiples formatos posibles)
    const modLower = (modifier || '').toLowerCase();
    const typeLower = (turnType || '').toLowerCase();
    const isLeft = modLower.includes('left') || typeLower.includes('left');
    const isRight = modLower.includes('right') || typeLower.includes('right');
...
// Rotar el mapa según heading (desactivado - causa problemas de renderizado)
function rotateMap(heading) {
    // Deprecated: use setMapRotation instead
    mapRotation = heading;
}

// ==========================================
// ROUTE RECALCULATION & GEOMETRY
// ==========================================

let lastRecalculationTime = 0;
const RECALCULATION_COOLDOWN = 15000; // 15 segundos entre recálculos

function checkOffRoute(userLatLng) {
    if (!routeCoordinates || routeCoordinates.length === 0) return;

    // Encontrar la distancia mínima a la ruta
    let minDistance = Infinity;

    for (let i = 0; i < routeCoordinates.length - 1; i++) {
        const p1 = L.latLng(routeCoordinates[i]);
        const p2 = L.latLng(routeCoordinates[i + 1]);
        const dist = distanceToLineSegment(userLatLng, p1, p2);
        if (dist < minDistance) {
            minDistance = dist;
        }
    }

    // Si nos desviamos más de 50 metros
    if (minDistance > 50) {
        const now = Date.now();
        if (now - lastRecalculationTime > RECALCULATION_COOLDOWN) {
            console.log(`⚠️ Desvío detectado: ${Math.round(minDistance)}m. Recalculando...`);
            showNotification(`⚠️ Desvío de ${Math.round(minDistance)}m - Recalculando ruta...`);
...
async function recalculateRoute(userLatLng) {
    ...
    markerA = L.marker(userLatLng, {
        icon: createMarkerIcon('A', '#22c55e'),
        draggable: true // Permitir ajustar si falla el GPS
    }).addTo(map);

    // Actualizar input de búsqueda (opcional, visual)
    searchInputA.value = "📍 Mi ubicación";

    // Recalcular ruta silenciosamente
    try {
        if (routeType === 'offroad') {
            await calculateOffroadRoute(userLatLng, markerB.getLatLng());
        } else {
            await calculateMixedRoute(userLatLng, markerB.getLatLng());
        }

        // Si estamos navegando, intentar mantener el zoom y centro
        if (isNavigating) {
            // Actualizar UI de navegación con nuevos datos
            updateNavigationUI();
        }
    } catch (error) {
...
// Distancia de un punto P a un segmento de línea A-B
function distanceToLineSegment(p, a, b) {
    ...
    // Distancia euclidiana aproximada en grados, convertir a metros
    // 1 grado latitud ~= 111km
    // Ajuste simple: sqrt(dx^2 + dy^2) * 111000 * cos(lat)
    const distDeg = Math.sqrt(dx * dx + dy * dy);
    return distDeg * 111139; // Aproximación estándar metros por grado
}
...
function getInstructionText(type, modifier) {
    const instructions = {
        ...
        'fork': modifier?.includes('left') ? 'Toma el desvío izquierdo' : 'Toma el desvío derecho',
        'merge': 'Incorpórate',
    ...
    if (type === 'turn' && modifier) {
        return instructions.turn[modifier] || 'Continúa';
    }
    return instructions[type] || 'Continúa recto';
}
...
function centerOnUser() {
    ...
    if (vehicleMarker) {
        ...
        target = userMarker.getLatLng();
    } else if (lastPosition) {
        target = lastPosition;
    }

    if (!target) {
        showNotification('📍 No hay ubicación GPS disponible');
        return;
    }

    // Usar offset si estamos navegando
    if (isNavigating) {
        // Centrar mapa ADELANTE de la posición (efecto Waze)
        const offsetCenter = calculateDestinationPoint(target, heading, 150);
...
function clearAll() {
    ...
    updateButtonStyles();
    showNotification('🗑️ Mapa limpiado');
}
    ...
    if ('geolocation' in navigator) {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                markerA.bindPopup('<b>Tu ubicación actual</b>');

                searchInputA.value = '📍 Mi ubicación actual';
                clearSearchA.style.display = 'block';

                map.setView(latlng, 15);
                showNotification('📍 Ubicación establecida como punto de inicio');
            },
            (error) => {
                showNotification('⚠️ No se pudo obtener tu ubicación');
            },
            { enableHighAccuracy: true }
        );
    }
...
// Función de síntesis de voz para notificaciones
function speak(text) {
